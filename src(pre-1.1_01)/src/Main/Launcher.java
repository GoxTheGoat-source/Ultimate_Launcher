...
public class Launcher {
    ...
    private static void preparerEtLancer() {
        ...
        try {
            ...
            // Utilisation d'une liste pour joindre le classpath proprement
            String cp = jar.getAbsolutePath() + File.pathSeparator + 
                        String.join(File.pathSeparator, Arrays.stream(new File(LIB_DIR).listFiles())
    ...
    private static void extraireNatives(File jarFile, File destinationDir) throws IOException {
        // On définit les extensions de bibliothèques natives pour Win, Mac et Linux
        List<String> extensionsNatives = Arrays.asList(".dll", ".dylib", ".so");
        ...
        try (ZipInputStream zis = new ZipInputStream(new FileInputStream(jarFile))) {
            ...
            while ((entry = zis.getNextEntry()) != null) {
                ...
                // On vérifie si le fichier se termine par l'une des extensions cibles
                boolean estNative = extensionsNatives.stream().anyMatch(name::endsWith);

                if (estNative) {
                    // Utilisation de .getName() pour éviter de recréer l'arborescence interne du ZIP
                    File outFile = new File(destinationDir, new File(name).getName());
                    ...
                    // Optionnel : donner les droits d'exécution sur Unix
